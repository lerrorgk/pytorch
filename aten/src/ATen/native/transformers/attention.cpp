...
_scaled_dot_product_flash_attention_cpu(
  ...
  if (at::cpu::is_amx_tile_supported() && Context::hasOneDNNGraph() &&
      (use_onednn_graph_sdpa != nullptr && std::strcmp(use_onednn_graph_sdpa, "1") == 0) &&
      (query.scalar_type() == c10::ScalarType::BFloat16) && (is_causal == false) &&
      (((attn_mask.has_value()) && (attn_mask.value().scalar_type() != at::ScalarType::Bool)) ||
      (!attn_mask.has_value()))) {
    use_onednn_graph = true;
